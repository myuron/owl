...
| iframe 内のヒント・focus 検知 | UserScript は全フレーム注入するが、クロスオリジン iframe とのメッセージングは要検証 | M5 で検証。MVP ではメインフレームのみ動作保証とする。**M5 実装時に §9 の新規 webkit6/javascriptcore API を存在確認済み**(M1 と同手法の `examples/` プローブ、webkit6 0.6.1):`UserContentManager::{new, add_script, register_script_message_handler(name, Option<world>)->bool, connect_script_message_received(Option<detail>, Fn(&Self, &javascriptcore::Value))}`・`UserScript::new(source, UserContentInjectedFrames::AllFrames, UserScriptInjectionTime::Start, allow[], block[])`・`javascriptcore::Value::to_str()->glib::GString`・`WebView::builder().user_content_manager()` / `WebView::user_content_manager()->Option<_>`。全て存在し欠落なし。`owlHints.*` の駆動は main world・メインフレームの `evaluate_javascript` で行うため、ヒント表示は仕様どおりメインフレーム限定。iframe 内ヒントの実挙動は checklist M5-23 で目視確認する |
| `about:blank` 上での JS 注入 | 空白ページでヒント等が動かなくても実害なし | 対処不要と確認のみ |
| Hint 開始後に page.js から応答が来ない | `f` は先に Hint へ遷移し `owlHints.start()` を fire-and-forget で評価する。UserScript が注入されない特殊ページ・Web プロセスクラッシュ後・ページが `window.owlHints` を削除/上書き・hint 表示中のページ遷移では `hint_none`/`hint_result` が来ず `-- HINT --` のままスタックする(全キー Stop) | MVP 許容。`Esc` で必ず Normal へ復帰できる(§7.2 の Hint+Esc)。checklist M5-22 に既知の制限として明記 |
| hint メッセージの偽装(main world 注入) | `"owl"` ハンドラは main world・全フレームに公開されるため、任意ページが `hint_result`/`hint_none` を送れる/`window.owlHints` を上書きできる | JS → Rust 側は **Hint モード中のみ hint 結果を受理**して偽装遷移を防ぐ(`input::install_hint_message_handler`、要求 3.3)。ページによる `owlHints` 上書き自体(hint の無効化・誤動作)は MVP 非対応。将来 isolated world(`UserScript::for_world` + `register_script_message_handler(name, Some(world))`)で隔離する |
| §10 focus メッセージの偽装(**M6 で実装**) | 同じ `"owl"` ハンドラに `{"type":"focus"}` を相乗りさせる(§10)。mousedown 相関の判定は page.js(信頼できないページ側)が行うため、任意ページが `postMessage({type:"focus"})` を偽装すれば Normal 中に owl を Insert へ強制遷移させうる(要求 3.3「スクリプト起因では Insert に入らない」の残余リスク) | JS → Rust 側は **Normal モード中のみ `focus` を受理**して Command/Hint/Insert 中の偽装を弾く(`input::install_hint_message_handler`、規約 6)。Normal 中の偽装 focus までは page.js の相関判定に依存するため防げないが、`Esc` で必ず Normal へ復帰でき実害は限定的なため MVP 許容(上記 hint 偽装と同種)。将来の isolated world 化で page.js ごと隔離する。checklist M6-20 に明記 |
