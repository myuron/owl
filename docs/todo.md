...
## サイクル 8: M6 insert モード自動移行

design.md §16 のマイルストーン **M6**「insert 自動移行」を実装タスクへ展開する。ゴールは
「ユーザー操作起因の focus でのみ Insert へ入り、`autofocus`・スクリプト起因では入らない」
(要求 3.3・§10)。

**スコープ境界:** hint 経由の入力欄 focus(`hint_result:input` → Insert)は M5 で実装済み(§9.2)。
M6 が追加するのは **マウスクリック経由の focus 検知**(§10: page.js の mousedown 相関 + focusin →
`{"type":"focus"}` → Normal のときのみ Insert)のみ。既存の `"owl"` script message handler を再利用し、
新規 webkit6 API は使わない(API 存在確認サイクル不要)。永続化・エラーページ等(§8)は M7。

**方針:** 純粋ロジック(`hints.rs` の `HintMessage::Focus` パース)と GTK/JS 結線(`page.js`・
`input.rs`)を分離(§4・§14)。純粋部は TDD + 100% coverage + mutants ゲート、結線部は手動確認
(checklist M6)。

### 8-A. 純粋 `parse_hint_message` の focus 対応(TDD、§10)

- [x] `docs/test.md §2.11` に `H-17` を追記(focus パース)。`H-13` を真の未知 type(`{"type":"bogus"}`)へ改訂
- [x] **Red**: `hints.rs` の `h13` を Focus 期待へ改訂 + `h17` を追加し、`HintMessage::Focus` 未実装で
      コンパイル/アサート失敗を確認(相異なる分岐を固定。規約 2・4)
- [x] **Green**: `HintMessage` に `Focus` を追加し、`parse_hint_message` に `Some("focus") => Focus` を実装
      (page.js が editable のときのみ送るため `editable` 値は再検証しない。§9.2 の既存方針)
- [x] **Refactor**: `just coverage`(command/keys/hints region・line 100%)・`just mutants`
      (survivor 0)・`cargo clippy`(-D warnings)/`cargo fmt --check` を通す

### 8-B. GTK/JS 結線(TDD 対象外、design §14。手動確認 = checklist)

- [x] `src/page.js`: §10 の focus 監視を追加。capture で `mousedown` のタイムスタンプ記録、`focusin` が
      直近 200ms 以内かつ `isEditable(e.target)`(既存関数を再利用)のとき `post({type:"focus",editable:true})`。
      autofocus・スクリプト `.focus()` は mousedown を伴わず通知されない。hint 確定の `.focus()`(§9)も
      同様に通知されず二重遷移しないことを doc に明記
- [x] `src/input.rs`: `install_hint_message_handler` を `Focus` 対応へ拡張。メッセージ種別ごとに
      「受理する現モード」を対にして検証(hint 結果は Hint 中のみ、`Focus` は Normal 中のみ Insert へ)。
      偽装 focus の MVP 許容リスクを doc に明記(規約 6)
- [x] `nix develop` 内で `cargo build`/`cargo clippy` が通ることを確認

### 8-手動確認(docs/checklist.md M6)

- [x] `docs/checklist.md` に M6 セクション(M6-01〜M6-21)を追記する
- [ ] `nix build` した `./result/bin/owl` で 入力欄クリック→Insert 自動移行・autofocus/スクリプト
      focus で Normal 維持・Esc 復帰・偽装 focus の既知の制限を**目視確認**する(checklist M6、ユーザー環境で実施)

### 8-完了条件

- [x] `docs/design.md §17` に §10 focus 偽装の MVP 許容リスクを追記する
- [x] `just ci`(fmt-check → lint → coverage → mutants → build)が緑になる
- [ ] `nix build` した起動で M6 の insert 自動移行が期待どおり動く(checklist M6 の目視確認)

---

## サイクル 9: M7 堅牢化

design.md §16 のマイルストーン **M7**「堅牢化」を実装タスクへ展開する。ゴールは「Cookie 永続化・
TLS Fail・新規ウィンドウ抑制・ダウンロードキャンセル・エラーページ/クラッシュ復帰」(§8.2〜8.6)。
M1 3-3 で §8 の全シグナル/API はコンパイルレベル存在確認済み(§17 解消済み)なので API 存在確認
サイクルは不要。

**スコープ境界:** M1 で最小結線した永続 `NetworkSession`(§8.2 の data/cache ディレクトリ)へ、
Cookie の SQLite 永続化(§8.2)・TLS Fail ポリシー(§8.3)を足す。`connect_create`(§8.4)・
`connect_download_started`(§8.5)・`connect_load_failed`/`load-failed-with-tls-errors`/
`web-process-terminated`(§8.6)を本結線する。TLS 例外許可 UI は作らない(要求 3.1)。設定
(`enable_developer_extras` 以外)は WebKit 既定のまま(§8.7)。次は M8 dogfooding。

**方針:** 純粋ロジック(`command::{error_page_html, download_blocked_message}`)と GTK/WebKit 結線
(`webview`・`window`)を分離(§4・§14)。純粋部は TDD + 100% coverage + mutants ゲート、シグナル
結線部は手動確認(checklist M7)。

### 9-A. 純粋ロジックの実装(TDD、§8.5・§8.6)

- [x] `docs/test.md §1.8` に新 ID(DL-01〜04・ERR-01〜03)を追記(DL メッセージ・エラーページ HTML)
- [x] **Red**: 未実装の `command::{download_blocked_message, error_page_html}` を参照する `dl01_*`〜
      `dl04_*`・`err01_*`〜`err03_*` を書き、コンパイルエラーを確認。相異なる分岐(URI basename の
      クエリ/フラグメント除去・空セグメントのフォールバック、HTML 全エスケープ分岐)を厳密に固定
      (規約 2・4: 素の `<script>` が残ると XSS になる値・truncate と round を分ける値を選ぶ)
...
- [x] `src/window.rs`: `build` で message_label 確定後に `webview::install_download_guard` を呼ぶ
      (§4: メッセージ欄が揃う位置で後付け結線)
- [x] `docs/design.md §4` の `command.rs` 記述に `error_page_html`/`download_blocked_message` を追記
- [x] `nix develop` 内で `cargo build`/`cargo clippy`(-D warnings)が通ることを確認。`nix build` 後、
      空の `XDG_DATA_HOME` で `about:blank` 起動 → panic/CRITICAL なし・`owl/cookies.sqlite` 生成を確認
      (§8.2 の永続化が実挙動で有効)

### 9-手動確認(docs/checklist.md M7)

- [x] `docs/checklist.md` に M7 セクション(M7-01〜M7-33)を追記する
- [ ] `nix build` した `./result/bin/owl` で TLS Fail のエラーページ・読み込み失敗のエラーページ・
      `r` リロード復帰・クラッシュ復帰・ポップアップ抑制・ダウンロードブロック表示・Cookie 永続化を
      **目視確認**する(checklist M7、ユーザー環境で実施)

### 9-完了条件

- [x] `just ci`(fmt-check → lint → coverage → mutants → build)が緑になる
- [ ] `nix build` した起動で M7 の堅牢化が期待どおり動く(checklist M7 の目視確認)

---

## サイクル 10: M8 dogfooding

design.md §16 のマイルストーン **M8**「dogfooding」を実装タスクへ展開する。ゴールは「チェックリスト
消化 → 1 週間常用」(§16-8・要求 §3.4)。**M8 は新規実装を持たない最終検収**で、純粋ロジックも GTK 結線も
変更しない(§14: 結合・E2E は自動化せず、手動確認と 1 週間 dogfooding を検証とする)。したがって TDD
サイクル(Red/Green/Refactor)は無く、`just ci` は不変で緑のまま。

**スコープ境界:** M1〜M7 で実装済みの全機能(要求 §3.1〜3.3)を実機で消化し、致命的支障がないことを
1 週間の常用で確認する。M1 の目視確認は 2026-07-18 に完了済み。M2〜M7 は checklist の「(実施予定)」が
残っているため、これを消化するのが本サイクルの主作業。新機能追加・タブ管理(要求 §4)は MVP 完了後。

### 10-A. 文書準備(本 PR で実施)

- [x] `docs/checklist.md` に M8 節を追加する — M2〜M7 の未消化項目を束ねる消化トラッキング表(M8.1)・
      1 週間常用ログの雛形(M8.2)・致命的支障ログ(M8.3)・MVP 完了判定(M8.4、要求 §3.4)。design.md §16
      は M8 を既に定義済み・スコープ変更なしのため design.md の更新は不要

### 10-B. チェックリスト消化(TDD 対象外、design §14。ユーザー環境で実施)

- [x] `nix build` した `./result/bin/owl` で checklist M8.1 の対象(M2-10〜M2-13・M3-10〜M3-53・
      M4-10〜M4-32・M5-10〜M5-24・M6-10〜M6-23・M7-11/M7-20〜M7-33)を**目視消化**し、各参照節の「結果」列を
      実挙動で埋める(規約 9: 理想でなく実挙動を書く)。全消化で checklist M8-01 達成
      (2026-07-23 消化完了。大半 ✅、既知の制限で再現しにくい 5 項目は ⚪ 未遭遇。M4-32 は期待結果の
      記述ミス〔Esc/Enter 復帰〕を実挙動へ修正)

### 10-C. 1 週間常用(ユーザー環境で実施)

- [ ] 日常閲覧(検索・記事閲覧・ログインが必要なサービス)を owl 中心で 1 週間行い、checklist の
      1 週間常用ログ(M8-10〜M8-16)と致命的支障ログ(M8.3)を埋める(要求 §3.4)。タブ不在起因の不便は
      致命的支障に数えない

### 10-完了条件

- [ ] checklist M8-30〜M8-32 を満たし **MVP 完了**と判定する(要求 §3.4: 3.1〜3.3 の全項目が動作し、
      1 週間の常用で致命的支障がない)
